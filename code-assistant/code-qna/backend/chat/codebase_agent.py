...
def index_code(dir_path: str, embedding_type: str, force_index: bool) -> str:
    ...
    tools = composio_toolset.get_actions(actions=[Action.CODE_ANALYSIS_TOOL_CREATE_CODE_MAP])
...
def find_code_snippet(dir_path: str, query: str) -> str:
    ...
    tools = composio_toolset.get_actions(actions=[Action.CODE_ANALYSIS_TOOL_GET_RELEVANT_CODE])
    search_agent = Agent(
        role="Search Agent",
