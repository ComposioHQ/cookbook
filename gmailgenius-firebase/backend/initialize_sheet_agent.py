...
def createSheet(entityId: str):
    ...
    google_assistant = Agent(
        ...
        goal="Create a Google Sheet and update it with provided values and store the spreadsheet_id in the database.",
    ...
    create_google_sheet = Task(
        description=f"""
        Create a new Google Sheet titled '{sheetTitle}', batch update it with the following values starting from A1: {attributes},
        ...
        expected_output="Create sheet, store values, update spreadsheet_id in the database",
    )
