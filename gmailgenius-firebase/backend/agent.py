...
import requests
from firebase.init import get_user_by_username, update_row
...
def callback_new_message(event: TriggerEventData) -> None:
    ...
    user = get_user_by_username(event.clientUniqueUserId)
    uid = user.get("uid")
    keywords = user.get("keywords")
    attributes = user.get("attributes")
    sheetId = user.get("sheetsConfig").get("spreadsheet_id")
    sheetName = "Sheet1"
    row = user.get("sheetsConfig").get("row")

    # Tools
    composio_toolset = ComposioToolSet(output_dir=Path.home()/"attachments",entity_id=event.clientUniqueUserId)
    ...
    process_new_email = Task(
        description=f"""
        1. Check if the email subject or body in the payload: {payload} contains any of these keywords: {keywords}.
        ...
        5. Store the identified attributes in Google Sheet with ID {sheetId}, sheet name {sheetName}, in cell A{row}.
